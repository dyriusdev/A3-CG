...
@onready var world_mesh: Node3D = $WorldMesh

@export var border_size : int = 40
...
func make_hallway(hallway_graph : AStar2D) -> void:
	...
	astar.region = Rect2i(Vector2i.ZERO, Vector2i.ONE * border_size)#Vector2i.ONE * border_size
	astar.update()
	...
	for h in hallways:
		var pos_from : Vector2i = Vector2(h[0].x, h[0].z)
		var pos_to : Vector2i = Vector2(h[1].x, h[1].z)
		var hall : PackedVector2Array = astar.get_point_path(pos_from, pos_to)
		
		for t in hall:
			var pos : Vector3 = Vector3(t.x, 0, t.y)
...
func make_room(recursion : int) -> void:
	...
	var start_position : Vector3i = Vector3i(randi() % (border_size - width + 1), 0, randi() % (border_size - height + 1))
	
	for r in range(-room_margin, height + room_margin):
		for c in range(-room_margin, width + room_margin):
			var pos : Vector3i = start_position + Vector3i(c, 0, r)
			if grid_map.get_cell_item(pos) == 0:
